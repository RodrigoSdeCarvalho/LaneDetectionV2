...
def process_frame(frame_tensor, detector, calibrator):
    ...
    image = calibrator.bird_eye_view(image)
...
def main():
    ...
    calibrator = Calibrator(None)  # Using default camera parameters

    # Process each frame
    for batch in test_loader:
        frame_tensor = batch['input_tensor'][0]
        processed_frame = process_frame(frame_tensor, detector, calibrator)
        
        cv2.imshow("TuSimple Center of Lane BEV Test", processed_frame)
